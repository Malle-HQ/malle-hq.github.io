...
export default function PixelGame({ leaderboard, loggedIn, onBack, onScore }: { leaderboard: GameScore[]; loggedIn: boolean; onBack: () => void; onScore: (score: number, look: GameLook) => Promise<void> }) {
  ...
  useEffect(() => { const context = previewRef.current?.getContext('2d'); if (!context) return; context.imageSmoothingEnabled = false; context.fillStyle = '#bcecff'; context.fillRect(0, 0, 150, 160); context.fillStyle = '#f4d08a'; context.fillRect(0, 132, 150, 28); drawPixelPerson(context, 68, 94, look) }, [look])
