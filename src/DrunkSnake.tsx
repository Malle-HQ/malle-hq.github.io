...
export type SnakeScore = { profileId: string; name: string; avatarUrl?: string | null; score: number }
const GRID = 20
const cocktails = ['🍹', '🍸', '🍷', '🥂']
...
export default function DrunkSnake({ leaderboard, onBack, onScore }: { leaderboard: SnakeScore[]; onBack: () => void; onScore: (score: number) => Promise<void> }) {
  ...
  const stop = useCallback(() => { runningRef.current = false; window.clearTimeout(timerRef.current); setRunning(false); setFinished(true); const nextBest = Math.max(best, scoreRef.current); setBest(nextBest); localStorage.setItem('malle-hq-snake-best', String(nextBest)); void onScore(scoreRef.current) }, [best, onScore])
